...
def search_events_example():
    """Example: Search for events using natural language"""
    print("=" * 60)
    print("EXAMPLE 1: Search Events")
    print("=" * 60)
    
    query = "I'm looking for a weekend bazaar in Jakarta with affordable booth prices under 3 million rupiah"
    ...
    print(f"\nFound {len(result.get('matches', []))} matching events:")
    for i, event in enumerate(result.get('matches', [])[:3], 1):
        ...
        print(f"   Price: Rp {event.get('booth_price', 'N/A'):,}")
...
def get_all_events_example():
    """Example: Retrieve all available events"""
    print("\n" + "=" * 60)
    print("EXAMPLE 4: Get All Events")
    print("=" * 60)
    ...
    print(f"\nTotal events available: {result.get('total', 0)}")
    ...
    if events:
        ...
        for i, event in enumerate(events[:3], 1):
            ...
            print(f"   Location: {event.get('location', 'N/A')}")
            print(f"   Booth Price: Rp {event.get('booth_price', 'N/A'):,}")
